// Binary raftadminwq is a CLI interface to the RaftAdmin gRPC service.
package main
...
var protoTypes = []protoreflect.ProtoMessage{
	...
	&pb.LastContentResponse{},
	&pb.LastContentRequest{},
	&pb.LastIndexRequest{},
...
func do() error {
	...
	methods := pb.File_raftadminwq_proto.Services().ByName("RaftAdminWq").Methods()
	...
	if flag.NArg() < 2 {
		...
		return fmt.Errorf("Usage: raftadminwq <host:port> <command> <args...>\nCommands: %s", strings.Join(commands, ", "))
	}
	...
	if flag.NArg() != 2+len(fields) {
		...
		return fmt.Errorf("Usage: raftadminwq <host:port> %s %s", command, strings.Join(names, " "))
	}
	...
	log.Printf("%v",resp)
	log.Printf("%v ||  %v||  %v||","/RaftAdminWq/"+string(m.Name()),req.Interface(),resp)  //wq
	if err := conn.Invoke(ctx, "/RaftAdminWq/"+string(m.Name()), req.Interface(), resp); err != nil {
		log.Printf("invoke err: %v",err)

		return err
	}
	log.Printf(" test resp: %v ",resp)
	...
	if f, ok := resp.(*pb.Future); ok {
		c := pb.NewRaftAdminWqClient(conn)
